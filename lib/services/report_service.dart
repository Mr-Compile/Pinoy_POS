...
class ReportService {
  ...
  ///
  /// Business rule: only Staff may submit reports (`submit_reports`), and a
  /// report can only be submitted by the user who created it. The Owner
  /// reviews submissions — they never author or submit staff reports.
  Future<bool> submitReport(int id) async {
    if (!_sessionManager.hasPermission('submit_reports')) return false;

    final report = await _exportHistoryRepository.getById(id);
    if (report == null) return false;

    final currentUser = _sessionManager.currentUser;
    if (report.createdBy != currentUser?.id) return false;
  ...
  ///
  /// "My Reports" is a Staff concept: it lists the report history of the
  /// authenticated report author. The Owner is not a report author — they
  /// review staff submissions via [getSubmittedReports] instead, so this
  /// always returns an empty list for non-authoring roles.
  Future<List<ExportHistory>> getMyReports() async {
    if (!_sessionManager.hasPermission('submit_reports')) return [];
  ...
  ///
  /// This creates a report record authored by the current user, so it is
  /// restricted to report-authoring roles (`submit_reports`). The Owner
  /// imports sales records through the Sales screen instead — importing a
  /// file here would incorrectly make them the author of a staff report.
  ///
